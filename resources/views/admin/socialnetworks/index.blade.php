	...
	<div class="container pt-lg-5 mt-5">
		<div class="row pt-3 pb-lg-0 pb-xl-0">
			<div class="col-lg-6 pt-4 mb-5 mb-lg-0">
				...
				<h1 class="font-weight-bold text-10 text-xl-12 line-height-2 mb-3">Redes Sociales</h1>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($socialNetwork->active)
                                                    <a class="btn btn-sm btn-success" href="{{ route('SocialNetworks.edit',$socialNetwork->id) }}"> SI</a>
                                                @else
                                                    <a class="btn btn-danger btn-sm" href="{{ route('SocialNetworks.edit',$socialNetwork->id) }}"> NO</a>
                                                @endif
                                            </td>

                                            <td>
                                                <form action="{{ route('SocialNetworks.destroy',$socialNetwork->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('SocialNetworks.show',$socialNetwork->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('SocialNetworks.edit',$socialNetwork->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
                                                    <!--
