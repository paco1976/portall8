			...
			<div class="row pt-3 pb-lg-0 pb-xl-0">
				<div class="col-lg-6 pt-4 mb-5 mb-lg-0">
					<ul class="breadcrumb font-weight-semibold text-4 negative-ls-1">
						...
						<li class="text-color-primary"><a href="{{ route('publicacion') }}">Panel de control</a></li>
						
					</ul>
					<h1 class="font-weight-bold text-10 text-xl-12 line-height-2 mb-3">MENSAJE DEL PROFESIONAL</h1>
		...
		<div class="container py-2">
			<div class="row mt-3 pb-4">
				<div class="col text-center">
					<h2 class="font-weight-bold mb-0">Mensajes de {{$user->name }} {{$user->last_name }} por la publicación de {{ $publicacion->categoria->name }} </h2>
				</div>
			</div>
			...
			@foreach($mensajes_all as $mensaje)
				...
				@endif
				@endforeach
				<form class="contact-form"  action="{{ route('publicacion_mensaje_respuesta', ['hash'=> $interactionhead->hash]) }}" method="POST" enctype="multipart/form-data" >
					{{ method_field('PUT') }}
					@csrf
					...
					<div class="row">
						<div class="form-group col">
                            ...
                            <a href="{{ route('publicacion_consultas', ['publicacion_hash'=> $publicacion->hash]) }}" class="btn btn-dark btn-modern">Volver</strong></a>
						</div>
					</div>
				</form>
